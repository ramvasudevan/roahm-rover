...
def solver_opt_k_FRS(solver_obj):
    ...
    t_opt = 0.7           #   Min time alloted to do optimization. If less than this time 
                          #   before t_plan will not do optimization.
    t_latency = 0.15      #   Unusable time at the end of the planning period to 
			  #   account for latency. This time is subtracted from 
			  #   the computed optimization time allowed.
    nlp.addOption('tol',0.1)
    nlp.addOption('acceptable_iter',3)
    nlp.addOption('acceptable_tol',0.15)
	...
	nlp.addOption('max_cpu_time',round(t_plan-t_j_to_now-t_latency,2)) 
	print(round(t_plan-t_j_to_now-t_latency,2))
...
def algorithm_main(oc):
    ...
    #   Decide what FRS to use. Uses a step approach
    ind_FRS = np.where(vel_range-opt_result[0]>0)[0][-1]
    w_p = np.squeeze(frs['poly'][0][0][0][0][ind_FRS])
    w_c = np.squeeze(frs['poly'][0][0][1][0][ind_FRS])
    ...
    oc_box_size = 4.0       #   Centered box area side length, (m), to register occupied 
		            #   grid points
    oc_1 = 15               #   Confidence level criteria of oc grid point 0(none)-100(max)
...
if __name__ == '__main__':
	...
	opt_result = [0,0]
        ...
        frs = sio.loadmat(
            '/home/nvidia/RTD_ws/src/autonomous_mode/Mat_Files/FRS/rover_FRS_many_deg_10.mat')
	vel_range = np.squeeze(frs['poly'][0][0][2][0])
