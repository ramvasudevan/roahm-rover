...
def gen_buf_discrete(oc_box_xy,oc_wh,oc_1,data,oc_buffer,oc,xy0,res):
    ...
    #
    obs = []
    tmp4 = np.zeros((oc_wh[0]*oc_wh[1],2))
    q = 0
    ...
    for x in oc_box_xy[0]:
		...
		#   as in FRS computation. 
                obs_FRS_0 = obs_map_2_FRS(res*x+xy0[0],res*y+xy0[1],pose_future[0:3])
		obs_FRS = [obs_FRS_0[0]+initial_x,obs_FRS_0[1]+initial_y]
		...
		and obs_FRS[1] >= box_FRS[2] and obs_FRS[1] <= box_FRS[3]):
                    obs.append(obs_FRS) 
		    tmp4[q,:] = [obs_FRS[0],obs_FRS[1]]
		    q = q+1

    np.save('/home/nvidia/RTD_ws/src/autonomous_mode/utility/jan14',tmp4)
    return obs
